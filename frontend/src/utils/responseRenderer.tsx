...
export const CodeBlock = ({
  ...
  return (
    <div
      className={`relative group rounded-lg overflow-hidden my-4 border ${
    ...
    >
      ...
      <SyntaxHighlighter
        ...
        style={
          isDarkMode ? vscDarkPlus : (vs as { [key: string]: CSSProperties })
        }
        customStyle={{
          margin: 0,
          padding: "1rem",
    ...
    <div className="markdown-content prose prose-slate dark:prose-invert max-w-none">
      <ReactMarkdown
        ...
        components={{
          ...
          th: ({ node, ...props }) => (
            <th
              className="px-4 py-3 text-left text-sm font-semibold text-gray-100"
              {...props}
            />
          ),
          td: ({ node, ...props }) => (
